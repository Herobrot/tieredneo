...
public class TieredGameplayEvents {
    ...
    public static void onBreakSpeed(PlayerEvent.BreakSpeed event) {
        ...
        if (digSpeedBonus > 0) event.setNewSpeed((float) (event.getNewSpeed() + digSpeedBonus));
    }
    ...
    public static void onCriticalHit(CriticalHitEvent event) {
        ...
        if (critChance > 0 && player.getRandom().nextFloat() < critChance) {
            ...
            if (event.getDamageMultiplier() <= 1.0f) event.setDamageMultiplier(1.5f);
        }
    }
    ...
    public static void onProjectileJoin(EntityJoinLevelEvent event) {
        if (!event.getLevel().isClientSide() && event.getEntity() instanceof AbstractArrow arrow && arrow.getOwner() instanceof Player player) {
            ...
            if (rangeBonus > 0) arrow.setBaseDamage(arrow.getBaseDamage() + rangeBonus);
        }
    }
}