...
import com.herobrot.tieredneo.gson.*;
...
public class AttributeDataLoader extends SimplePreparableReloadListener<Map<ResourceLocation, PotentialAttribute>> {
    ...
    @Override
    protected @NotNull Map<ResourceLocation, PotentialAttribute> prepare(ResourceManager manager, @NotNull ProfilerFiller profiler) {
        ...
        manager.listResources(DATA_FOLDER, id -> id.getPath().endsWith(".json")).forEach((fileId, resource) -> {
            try (InputStreamReader reader = new InputStreamReader(resource.open(), StandardCharsets.UTF_8)) {
                ...
                TieredNeo.LOGGER.error("[TieredNeo]: Failed to parse item_attributes file {}", fileId, e);
            }
        });
    ...
    @Override
    protected void apply(@NotNull Map<ResourceLocation, PotentialAttribute> prepared, @NotNull ResourceManager manager, @NotNull ProfilerFiller profiler) {
        this.itemAttributes = Map.copyOf(prepared);
        TieredNeo.LOGGER.info("[TieredNeo]: Applied {} item attributes.", this.itemAttributes.size());
    }

    public Map<ResourceLocation, PotentialAttribute> getItemAttributes() { return itemAttributes; }
